...
# Look for poles in the 4^3 first n triplets
...
for guess in guesses:
	...
	poleq2ran = 500
	polex2ran = poleq2ran*(L/(2*np.pi))**2

	while index1 < len(poles_sorted)-1:
		# Only add poles inside the q2 range
		if poles_sorted[index1] > x2fmax + polex2ran:
			break
		elif poles_sorted[index1] < x2fmin - polex2ran:
			index1 +=1
			continue
		else:
	...
	x = np.zeros(points_f)
	y = np.zeros(points_f) + I*np.zeros(points_f)

	for x2 in xrange(0,points_f):
		if x2 % 10 == 0:
			print x2
		x[x2] = x2/(points_f-1.0)*(x2fmax - x2fmin) + x2fmin
		y[x2] = ff(l,m,x[x2],poles)
	...
	np.save(ffolder+filename,y)

	filename = 'x2-'+filename 

	np.save(ffolder+filename,x)

	filename = 'poles-'+ filename[3:]

	np.save(ffolder+filename,poles[0:num_poles])
